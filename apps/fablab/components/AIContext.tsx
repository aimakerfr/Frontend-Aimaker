...
const AIContext: React.FC = () => {
  ...
  const getFixedTitle = (item: Product): string => {
    ...
    if (item.type === 'api_key_html_injector') return t.products.fixed.apiKeyHtmlTitle || 'API Key Injection to HTML';
    if (item.type === 'style_transfer_maker') return t.products.fixed.styleTransferTitle;
    return t.products.fixed.translationTitle;
  };
  ...
  const getFixedDescription = (item: Product): string => {
    ...
    if (item.type === 'profile_b2b_maker') {
      return t.products.fixed.profileB2BDesc || 'B2B pipeline with OSINT, persona, matching, and personalized landing generated by AI.';
    }
    if (item.type === 'api_key_html_injector') {
      return t.products.fixed.apiKeyHtmlDesc || 'Store API keys per product, use a guided prompt, and deploy your HTML through the AI proxy.';
    }
    if (item.type === 'style_transfer_maker') return t.products.fixed.styleTransferDesc;
    return t.products.fixed.translationDesc;
  };
