...
const Body: React.FC<BodyProps> = ({ state, onUpdate }) => {
  ...
  const inputStyle = "w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 transition-all text-[15px] text-slate-700 placeholder:text-slate-300 bg-white shadow-sm";
            ...
            <button
              ...
              className={`flex flex-col items-center justify-center p-8 rounded-2xl border-2 transition-all group ${
                state.projectType === type.id
                  ? 'bg-[#eff6ff] border-[#3b82f6] text-[#2563eb] shadow-md shadow-blue-100'
                  : 'bg-white border-slate-100 text-slate-400 hover:border-slate-200 hover:bg-slate-50'
              }`}
            >
              <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-4 transition-all ${
                state.projectType === type.id ? 'bg-[#3b82f6] text-white' : 'bg-slate-50 text-slate-300 group-hover:bg-white'
              }`}>
                <i className={`fa-solid ${type.icon} text-xl`}></i>
              </div>
              <span className="font-bold text-sm tracking-wide uppercase text-center">{type.label}</span>
            </button>
          ))}
        </div>
      </section>
