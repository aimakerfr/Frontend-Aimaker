...
import { Send, Bot, User, Sparkles, BookOpen, ChevronDown, ChevronUp, Target, Layers, ArrowLeft } from 'lucide-react';
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ messages, onSendMessage, isLoading, sourceSummary, isSummaryLoading, onBack }) => {
    const { t } = useLanguage();
        ...
        <div className="flex flex-col h-full bg-white relative">
            {onBack && (
                <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50">
                    <button
                        onClick={onBack}
                        className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-indigo-600 hover:bg-white rounded-lg transition-all"
                    >
                        <ArrowLeft size={18} />
                    ...
                    <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="font-black text-gray-800">Chat con Fuentes</span>
                    </div>
                </div>
            )}
            <div className="flex-1 overflow-y-auto px-4 py-6 md:px-12 space-y-6 no-scrollbar">
                                ...
                                {isSummaryLoading ? <div className="space-y-6 animate-pulse"><div className="h-6 bg-gray-100 rounded-full w-3/4"></div></div> : sourceSummary && (
                                    <>
                                        ...
                                        <div className="space-y-6">
                                            <div className="flex items-center gap-2 text-gray-500 font-bold text-xs uppercase tracking-widest"><Layers size={16} /> {tc.breakdown}</div>
                                                ...
                                                {sourceSummary.sourcesAnalysis.map((source, idx) => (
                                                    <div key={idx} className="flex flex-col border border-gray-100 rounded-2xl p-6 bg-white shadow-sm">
                                                        <div className="flex items-center justify-between mb-4">
                                                            ...
                                                            <span className="text-[9px] font-black uppercase px-2 py-0.5 bg-gray-50 text-gray-500 rounded-md border border-gray-100">{source.type}</span>
                                                        </div>
                                                        <p className="text-gray-700 text-sm mb-6">{source.summary}</p>
                                                        <div className="mt-auto space-y-6">
                                                            <div><div className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-3">{tc.topics}</div>
                                                                 <div className="flex flex-wrap gap-2">{source.keyTopics.map((topic, i) => <span key={i} className="text-[10px] px-2.5 py-1 bg-gray-50 text-gray-700 rounded-full font-bold border border-gray-100">{topic}</span>)}</div>
                                                            </div>
                                                            <div className="pt-4 border-t border-gray-50">
                                                                <div className="text-[10px] font-black text-indigo-500 uppercase tracking-widest mb-3">{tc.questions}</div>
                                                                <div className="space-y-2">
                                                                    {source.suggestedQuestions.map((q, i) => (
                                                                        <button key={i} onClick={() => handleSuggestedClick(q)} className="flex items-start gap-2 w-full text-left text-[11px] text-indigo-600 hover:text-indigo-800 transition-colors">
                                                                            <span className="mt-1 w-1 h-1 rounded-full bg-indigo-400"></span><span className="flex-1">{q}</span>
                                                                        </button>
                                                                    ))}
                                                                </div>
                                                            </div>
                ...
                {messages.length === 0 && !isLoading ? (
                    <div className="flex flex-col items-center justify-center py-24 opacity-30">
                        ...
                        <h3 className="text-2xl font-black text-gray-900 tracking-tighter">{tc.start}</h3>
                        <p className="text-sm text-gray-700 text-center mt-3 font-medium">{tc.startSub}</p>
                    </div>
                ) : (
                    messages.map((msg) => (
                ...
                {isLoading && (
                    <div className="flex gap-5 justify-start animate-in fade-in slide-in-from-bottom-2 max-w-4xl mx-auto w-full">
                        <div className="w-10 h-10 rounded-2xl bg-indigo-100 flex items-center justify-center text-indigo-600"><Bot size={20} /></div>
                        <div className="bg-white border border-gray-100 p-6 rounded-[2rem] rounded-tl-none shadow-sm flex items-center gap-4">
                            <div className="flex gap-1.5"><span className="w-2 h-2 bg-indigo-500 rounded-full animate-bounce"></span><span className="w-2 h-2 bg-indigo-500 rounded-full animate-bounce delay-150"></span><span className="w-2 h-2 bg-indigo-500 rounded-full animate-bounce delay-300"></span></div>
                            <span className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em]">{tc.synthesizing}</span>
                        </div>
                    </div>
                )}
