...
import { 
    getNotebookModules, 
    assignNotebookModule, 
    unassignNotebookModule, 
    downloadNotebookIndex,
    type NotebookModule, 
    type ModuleType 
} from '@core/notebooks';
import { getNotebookSources, type NotebookSourceItem } from '@core/notebooks';
...
const NotebookModulesView: React.FC = () => {
    const { id } = useParams<{ id: string }>();
    const navigate = useNavigate();
    const notebookId = parseInt(id || '0');

    const [modules, setModules] = useState<NotebookModule[]>([]);
    const [htmlSources, setHtmlSources] = useState<NotebookSourceItem[]>([]);
    ...
    const getModulesByType = (type: ModuleType): NotebookModule[] => {
    ...
    const handleAssign = async (moduleType: ModuleType, sourceId: number) => {
        ...
        try {
            await assignNotebookModule(notebookId, moduleType, sourceId);
            await loadData();
    ...
    const handleUnassignAll = async (moduleType: ModuleType) => {
        setAssigning(moduleType);
        try {
            await unassignNotebookModule(notebookId, moduleType);
            await loadData();
        } catch (err) {
    ...
    const handleUnassignOne = async (moduleType: ModuleType, moduleId: number) => {
        setAssigning(moduleType);
        try {
            await unassignNotebookModule(notebookId, moduleType, moduleId);
            await loadData();
        } catch (err) {
            console.error('Error unassigning module:', err);
        }
    ...
    const handleDownloadIndex = async () => {
        setDownloading(true);
        try {
            await downloadNotebookIndex(notebookId);
        } catch (err) {
            ...
            <div className="bg-white border-b border-gray-100 px-6 py-4 sticky top-0 z-50">
                <div className="max-w-4xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <button 
                            onClick={() => navigate(`/dashboard/notebook/${notebookId}`)} 
    ...
    useEffect(() => {
        const fetchHtmlModules = async () => {
            ...
            for (const mod of modules) {
                ...
                const url = `${apiUrl}/api/v1/notebook-modules/module/${mod.id}/html`;
...
export default NotebookModulesView;
