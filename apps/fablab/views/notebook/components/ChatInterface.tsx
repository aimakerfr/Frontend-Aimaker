...
import { Send, Bot, User, Sparkles, BookOpen, ChevronDown, ChevronUp, Target, Layers, FileSearch, ListOrdered } from 'lucide-react';
            ...
            <div className="p-8 border-t border-gray-50 bg-white/80 backdrop-blur-xl">
                <div className="max-w-4xl mx-auto space-y-3">
                    ...
                    <div className="flex gap-2 items-center">
                        <button
                            type="button"
                            onClick={() => handleArtifactClick(tc.artifacts?.analyzeSources || 'Analiza las fuentes')}
                            disabled={isLoading}
                            className="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-indigo-50 to-purple-50 hover:from-indigo-100 hover:to-purple-100 border border-indigo-200 rounded-xl text-xs font-semibold text-indigo-700 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
                        >
                            <FileSearch size={14} />
                            <span>{tc.artifacts?.analyzeSources || 'Analiza las fuentes'}</span>
                        </button>
                        <button
                            type="button"
                            onClick={() => handleArtifactClick(tc.artifacts?.summarizeIdeas || 'Resume las ideas principales')}
                            disabled={isLoading}
                            className="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-blue-50 to-cyan-50 hover:from-blue-100 hover:to-cyan-100 border border-blue-200 rounded-xl text-xs font-semibold text-blue-700 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
                        >
                            <ListOrdered size={14} />
                            <span>{tc.artifacts?.summarizeIdeas || 'Resume las ideas principales'}</span>
                        </button>
                    </div>
